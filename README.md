...
- **펫** — 펫 선택(**대분류: 포켓몬 / 동물 / 메이플스토리**로 묶인 팝업 — 각 그룹 헤더 아래에
  펫이 들여쓰기되어 뜨고, 아직 펫이 없는 카테고리는 "준비 중"으로 보입니다), 진화 사용, 경험치 바 항상 표시, 모든 경험치 초기화
- **상태 소스** — Claude Code / Orca / Claude Desktop
- **연동** — Claude Code 상태 훅(얼음/헤롱헤롱) 설치, 전체 디스크 접근 권한 열기
- **대전 / 노려보기** — **방해금지 모드**(켜면 남이 거는 대전·노려보기를 받지 않음), 같은 Wi-Fi에서 발견된 상대에게 대전 신청·노려보기
- **앱** — ConnorPet 종료
...
- **idle → 잠듦**: 채도/밝기를 크게 낮추고 위로 떠오르며 사라지는 "Zzz"를 얹었습니다. 할 일이
  없을 때는 그냥 자고 있는 걸로.
- **running**: 색 변화 없이 빠른 바운스 루프 그대로 — 포켓몬은 변경 없음. 단 **동물 대분류**는
  제자리 바운스만으론 달리는 느낌이 약해, 뒤로 **발자국(paw print) 트레일**이 남아 흘러가며
  사라지도록 얹었습니다(`paw_print_layer`, 펫보다 아래 레이어라 몸통에 안 겹침).
- **waiting → 얼음**(포켓몬): 한 포즈에 고정(=멈춰있음)하고 각진 얼음 결정 오버레이(모서리에 삐죽
  튀어나온 조각 포함)를 씌웠습니다. 채도만 낮췄던 이전 버전보다 "완전히 멈췄음"이 훨씬 명확하게
  전달됩니다.
- **waiting → 고개 갸웃 + `?`**(동물): 실제 동물한테 얼음(포켓몬 상태이상)은 어색해서, **동물
  대분류** 펫은 대기 상태를 **앉아서 고개를 갸웃하고 `?` 말풍선을 띄운 채 지시를 기다리는** 포즈로
  다르게 굽습니다. "블락됐어요, 어떻게 할까요?" 하고 주인을 기다리는 뉘앙스 — 얼음과 의미는 같지만
  (=너를 기다림) 동물답게 표현합니다. 지금은 비숑이 유일한 동물이지만, 새 동물도 같은 경로를 탑니다.
...
**대분류(category)**가 정합니다 — 포켓몬은 얼음/기본 달리기, 동물은 고개 갸웃/발자국 달리기 — 이
분류는 `PETS`(빌드)와 `AppDelegate.petCategories`(앱)가 공유합니다.
...
- **비숑 (Bichon)은 대전을 하지 않습니다** — 실제 반려견이라 동물보호 차원에서 뺐습니다. 비숑을 고르면 대전 메뉴가 "대전 안 함"으로 바뀌고, 신청도 걸 수 없으며, 남이 걸어온 대전은 조용히 거절됩니다(상대는 "거절됨"으로 봅니다). 발견·**노려보기**는 그대로 됩니다. 게이팅은 `AppDelegate.nonBattlePetSlugs` 한 곳에서 합니다.
...
Orca 자체 펫으로 쓰고 싶으면(Settings → Experimental → Pet → Import), 원하는 펫의 `<slug>.codex-pet/` 폴더(`totodile`/`ditto`/`charmander`/`squirtle`/`geodude`/`eevee`/`chikorita`/`torchic`/`togepi`/`tepig`/`snorlax`/`gengar`/`diglett`/`pikachu`)를 그대로 임포터에 지정하면 됩니다.
...
대전에서 내 펫이 **진화 전 모습**으로 나오던 문제가 있었습니다. 상대는 진화형으로
보이는데 내 쪽만 기본형이라 더 눈에 띄었습니다.

원인은 두 slug 를 섞어 쓴 것입니다.

| | 무엇인가 | 어디에 쓰나 |
|---|---|---|
| `selectedPetSlug` | 메뉴에서 고른 **기본형** | 경험치·이름을 저장할 신원 (진화해도 안 바뀌어야 함) |
| `displayedPetSlug` | 지금 **보이는** 모습 | 그림·종 이름 |

대전 화면이 앞쪽을 쓰고 있었습니다. 같은 혼동이 마우스 오버 종 이름과 설정의 "비우면
OO로 돌아가요" 안내에도 있어 함께 고쳤습니다. 상대 쪽은 원래부터 표시형을 광고하고
있어 정상이었습니다.

**진화형은 더 크게 그립니다** — 1단계 1.3배, 2단계 1.6배. 같은 크기로 그리면 진화한
보람이 화면에 없습니다. 발을 바닥에 두고 위로 키웁니다(가운데 기준으로 키우면 발이
바닥을 뚫습니다). 위로 넘쳐 잘리는 것은 괜찮지만, **좌우로 화면 밖에 나가면** 몸통이
뚝 잘려 연출이 아니라 버그로 보이므로 가로만 화면 안으로 밉니다.

상대의 단계는 slug 로 역산합니다 — 메시지에는 slug 만 오고 단계는 오지 않으므로,
진화 사슬에서 몇 번째인지가 곧 단계입니다.

```sh
CONNORPET_DEBUG_BATTLE=/tmp/b.png CONNORPET_DEBUG_STAGE=2 swift run
```

전투 화면을 PNG 로 떠서 확인합니다. `CONNORPET_DEBUG_STAGE` 로 진화 단계를,
`CONNORPET_DEBUG_OPP` 로 상대 펫을 지정합니다 — "진화 전 모습이 나온다" 같은 문제는
눈으로만 잡힙니다.
...
설정 › 펫 › **펫 이름** 에서 지금 고른 펫에 이름을 지어 줍니다. 비우고 Return 을 누르면
도감 이름(파이리 등)으로 돌아갑니다.

**기본형 slug 로 저장합니다.** 파이리에 "불꽃" 을 지어 두면 리자드·리자몽으로 진화해도
같은 이름입니다 — 진화는 같은 펫이 자란 것이지 다른 펫이 아닙니다.

지은 이름은 두 곳에 나옵니다.

- **마우스 오버** — 펫 아래 문구의 첫 줄이 이름, 둘째 줄이 경험치입니다. 경험치는 바로도
  보이지만 이름은 여기서만 보이므로 이름을 앞에 둡니다.

  두 줄이 되면서 창 폭 계산을 고쳤습니다. `intrinsicContentSize` 는 여러 줄에서 폭을
  모자라게 잡아 첫 글자(`E`)가 잘렸습니다. 폭을 넉넉히 주고 `sizeThatFits` 로 실제
  필요한 크기를 물어본 뒤 **올림**합니다 — 소수점이 남으면 창 프레임이 정수로 깎이면서
  마지막 픽셀이 사라집니다. 글자에 그림자(흐림 3)를 깔아 두어 가로 여백도 늘렸습니다.
  `label.maximumNumberOfLines = 0` 을 주지 않으면 둘째 줄이 통째로 사라집니다.

  ```sh
  CONNORPET_DEBUG_HOVER=/tmp/h.png swift run
  ```

  문구 창을 PNG 로 떠서 확인합니다. `CONNORPET_DEBUG_HOVER_TEXT` 로 원하는 문구를 넣어
  긴 이름·MAX 표기 등을 시험할 수 있습니다 — 글자가 잘리는지는 눈으로만 잡힙니다.
- **노려보기** — 상대 화면에 `OO의 불꽃이 노려봐요!` 로 뜹니다. 이름은 각자 기기에만
  있으므로 이것만은 메시지에 실어 보냅니다(slug 로는 알 수 없습니다).

### 조사를 이름에 맞춰 고릅니다

예전에는 도감 이름만 썼고 "파이리가" 처럼 모음으로 끝나는 것이 많아 `가` 를 문구에
박아 두었습니다. 사용자가 이름을 짓게 되면 "불꽃" 처럼 받침으로 끝나는 이름이 들어오고,
그대로 두면 `불꽃가 노려봐요` 가 됩니다.

한글 음절은 유니코드에서 `가`(0xAC00)부터 28개 단위로 종성이 돌아갑니다 — 나머지가 0
이면 받침이 없습니다(`KoreanParticle`). 한글이 아닌 글자로 끝나면 받침을 알 수 없으므로
받침 없음으로 봅니다(`Pika가` 가 `Pika이` 보다 덜 어색합니다). `로`/`으로` 는 받침이
`ㄹ` 일 때도 `로` 인 것이 예외입니다.

### 받은 이름은 믿지 않습니다

빈 문자열은 없는 것으로 보고 도감 이름을 씁니다. 우리 앱은 빈 이름을 저장하지 않지만
들어오는 값은 남의 기기에서 온 것이라, 그대로 쓰면 `OO의 가 노려봐요` 가 됩니다(실제로
그렇게 나왔습니다). 길이도 12자로 자릅니다 — 긴 문자열을 보내면 말풍선이 화면을
가로지릅니다.

```sh
CONNORPET_SELFTEST=petname swift run
```

저장(펫마다 따로 · 공백은 지움 · 길이 제한 · 다시 읽어도 유지)과 조사 선택을 확인합니다.
사용자가 지어 둔 이름은 건드리지 않고 검증용 저장소에 대고 돕니다.

## 노려보기

메뉴바 → **노려보기** 에서 같은 Wi-Fi 의 상대를 고르면, 그쪽 펫 옆에
`😠 OO의 파이리가 노려봐요!` 라는 **작은 말풍선**이 뜹니다 — 말풍선 안에는 **상대 펫
도트(얼굴)** 가 함께 보입니다. 업무를 방해하지 않도록 가운데 모달이 바로 튀어나오는
대신 이 말풍선만 조용히 뜨고(위 "모든 상호작용은 말풍선으로" 참고), **말풍선을 누르면**
그때 상대 펫 얼굴이 큼직하게 뜨는 모달로 이어집니다. 누르지 않으면 잠시 뒤 사라집니다.
노려보기는 응답을 기다리지 않습니다 — 보내고 연결을 닫습니다. 상대가 **방해금지 모드**
면 노려보기 자체가 전달되지 않고, 메뉴·설정창에서 그 상대는 "방해금지 중"으로 잠겨
보입니다.

### 얼굴은 주고받지 않고 각자 그립니다

말풍선 도트와 모달의 얼굴은 모두 `PetPortrait` 가 스프라이트의 **알파(투명도) 경계
위쪽 정사각형**을 잘라 만든 초상입니다(프레임 여백이 펫마다 달라 46%~83%로 제각각이라,
알파 경계를 먼저 구해야 어느 펫이든 같은 크기로 보입니다). 그림은 네트워크로 오가지
않고 메시지엔 slug 만 실려, 스프라이트는 두 앱 모두 번들에 갖고 있으므로 **각자 자기
번들에서** 찾아 그립니다(위조·용량 문제 방지). 우리 번들에 없는 펫(상대가 더 최신
버전)이면 도트 없이 문구만 뜹니다.
...
번들의 **모든** 펫(메뉴에 안 뜨는 진화형 포함)에 대해 초상이 정사각형으로, 알파 경계로
좁혀져 나오는지 봅니다. `CONNORPET_PORTRAIT_OUT=<디렉터리>` 를 주면 PNG 로 떨어뜨립니다.

광고하는 slug 는 메뉴에서 고른 기본형이 아니라 **지금 화면에 보이는 펫**입니다. 안
그러면 리자몽으로 진화해 놓고도 상대에게는 "파이리가 노려봅니다" 로 갑니다. 메뉴 선택도
진화도 `refreshDisplayedPet()` 을 지나므로 거기 한 곳에서 갱신합니다.
...
집중해야 할 때 같은 Wi-Fi의 다른 사람이 거는 **대전·노려보기를 받지 않게** 하는 스위치입니다. 세 곳 어디서든 켤 수 있고(같은 값이라 어디서 켜든 일치합니다):

- **펫 우클릭 메뉴** → `방해금지 모드` (체크 표시로 현재 상태)
- **메뉴바 아이콘** → `방해금지 모드`
- **설정 창** → `대전 / 노려보기` 섹션 맨 위 스위치

켜면:

- 들어오는 대전 신청·노려보기를 **아무 알림 없이 받지 않습니다.** (거절이 아니라 "안 받음" 이라, 상대 쪽은 무응답으로 조용히 정리됩니다.)
- 그 사실을 Bonjour TXT 레코드(`dnd`)로 광고해, **상대 앱은 나를 목록에 그대로 두되**(노출) 내 대전 신청·노려보기 항목을 **잠그고 "방해금지 중"** 이라고 밝힙니다 — 눌러도 소용없다는 걸 상대가 알 수 있습니다.
- **내가 남에게 거는 것**(신청·노려보기)은 막지 않습니다. 이 모드는 "받지 않음" 이지 "끊음" 이 아닙니다.

구버전 상대는 `dnd` 필드를 모르지만, 그래도 받는 쪽(내 앱)이 들어오는 상호작용을 끊으므로 실제로 방해받지 않습니다.

### 모든 상호작용은 말풍선으로 — 모달을 바로 띄우지 않습니다

이 앱은 업무 중 배경에 띄워 두는 것이라, 포커스를 뺏는 가운데 모달이 **바로** 튀어나오면 그 자체로 방해가 됩니다. 그래서 **남이 거는 상호작용·알림은 먼저 펫 옆 작은 말풍선으로** 알리고, **모달은 사용자가 그 말풍선을 누른 뒤에만** 뜹니다 — 노려보기를 받으면 상대 펫 도트가 든 말풍선(`😠 OO의 파이리가 노려봐요!`)이 뜨고 누르면 얼굴이 큼직한 모달로, 대전 신청도 펫 옆 작은 "Challenge" 말풍선·카운트다운으로 시작해 누르면 수락/거절 모달로 이어집니다.

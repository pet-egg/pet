...
이 신호는 **손쉬운 사용(Accessibility) 권한**이 필요합니다. **Claude Desktop 소스로 켤 때**(첫 실행
마법사에서 고르든, 메뉴바/설정에서 전환하든, 그 소스로 앱이 실행되든) 권한이 없으면 **설정으로 안내하는
알림**을 띄웁니다 — macOS 기본 권한 다이얼로그는 TCC 기록이 한 번 생기면 다시 안 뜨는 일회성이라,
**안 켜져 있는 동안엔 켤 때마다** 우리 알림으로 안내합니다(`AppDelegate.promptAccessibilityIfNeeded`).
알림의 **‘손쉬운 사용 설정 열기’** 를 누르면 pet 을 목록에 등록하고 `시스템 설정 › 개인정보 보호 및 보안
› 손쉬운 사용` 창을 바로 엽니다. 켜면 재실행 없이 바로 반영되고, 권한이 없는 동안에도 "생성 중" 감지만
빠진 채(실행/포그라운드 + 알림 기반 완료로) 계속 동작합니다.

> **업데이트하면 기존 손쉬운 사용 권한은 풀립니다(재승인 필요)**: 이 앱은 ad-hoc 서명(Developer ID 없음)
> 이라 빌드마다 코드서명 해시(cdhash)가 달라집니다. macOS TCC는 손쉬운 사용 권한을 이 해시에 묶어 두므로,
...
> 안정적인 Developer ID 서명 없이는 이 권한을 유지할 방법이 없어서, 워처가 켜질 때 **버전이 바뀌었는데
> 인증이 안 된 상태**면 `tccutil reset Accessibility`로 낡은(이미 무효인) 권한 기록을 지웁니다. 그러면 위
> 안내 알림/시스템 요청이 다시 떠서 **손쉬운 사용 목록에서 수동으로 껐다 켤 필요 없이 한 번에 재승인**하면
> 됩니다(정상 동작하는 권한은 절대 건드리지 않음). 구현: `ClaudeAXProbe.reconcileAccessibilityGrant()`.

...
동물 달리기용 `draw_paw_prints()`)로 PNG에 직접 구운 것이라, Swift 쪽 코드는 건드리지 않았습니다
...
- 진화 체인은 다음 도감 번호를 그대로 씁니다(메타몽·토게피·잠만보·팬텀은 진화 없음, 비숑은 포켓몬이 아니라 진화 없음, 이브이·디그다는 1단계만):

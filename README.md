...
`draw_hearts()`/`draw_zzz()`(+ 동물 대기용 `draw_bichon_sitting()`/`draw_question_bubble()`)로
PNG에 직접 구운 것이라, Swift 쪽 코드는 건드리지 않았습니다(애니메이션 우선순위/재생 로직은 그대로,
스프라이트 픽셀만 다시 구운 것). 어느 스킨을 쓸지는 펫의 **대분류(category)**가 정합니다 — 포켓몬은
얼음, 동물은 고개 갸웃 — 이 분류는 `PETS`(빌드)와 `AppDelegate.petCategories`(앱)가 공유합니다.

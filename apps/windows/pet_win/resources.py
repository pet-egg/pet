...
- 소스에서 그냥 실행: 저장소 정본 `assets/pets` 를 쓴다.
...
def _candidates():
    ...
    win_dir = os.path.dirname(here)                    # apps/windows/
    repo_root = os.path.dirname(os.path.dirname(win_dir))  # apps/windows → repo
    paths = []
    ...
    # 저장소 정본(assets/pets — mac 앱과 공유하는 단일 소스).
    paths.append(os.path.join(repo_root, "assets", "pets"))
    return paths

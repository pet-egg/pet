...
#
#   ./tooling/scripts/make_app.sh              # ~/Applications 에 설치 (기본값)
#   ./tooling/scripts/make_app.sh /Applications  # 설치 위치 지정
set -euo pipefail
...
# mac 타깃의 SwiftPM 번들 리소스(pets·effects·hooks)는 git 에 없다 — 정본
# (assets/·tooling/hooks/)에서 sync 로 채운다. 빌드 전에 반드시 먼저 돌린다.
echo "▸ 리소스 sync (assets → mac 미러)"
python3 "$REPO_ROOT/tooling/scripts/sync_assets.py"
APP="$INSTALL_DIR/ConnorPet.app"

# dmg(.github/workflows/build-release.yml)와 **반드시 같은 값**이어야 한다.

...
Usage:
  python3 tooling/scripts/install_claude_hooks.py            # install (or migrate)
  python3 tooling/scripts/install_claude_hooks.py --uninstall # remove connor-pet's entries
"""
import json
import os
import shutil
...
# 훅 핸들러 정본은 tooling/hooks/ 에 있다(앱 번들 사본과 공유하는 단일 소스).
HOOK_SCRIPT = os.path.join(os.path.dirname(HERE), "hooks", "pet_hook_status.py")

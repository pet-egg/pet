#!/usr/bin/env python3
"""
Builds each configured Pokémon's sprite sheet + manifest into the **정본**
`assets/pets/<slug>/{spritesheet.png,pet.json}` from PokeAPI's public gen5
battle sprites. 정본을 만든 뒤 `sync_assets.py` 를 호출해 mac 미러
(`apps/macos/Sources/ConnorPet/Resources/pets`)와 Orca 임포트 번들
(`dist/orca/<slug>.codex-pet`)을 정본에서 재생성한다 — 사본은 절대 손으로
편집하지 않으므로 드리프트가 생길 수 없다.

Requires: pillow (`pip install pillow`)

Usage: python3 tooling/scripts/build_sheet.py
"""
import json
import math
import os
...
# 펫 스프라이트의 **정본**. mac 미러(apps/macos/.../Resources/pets)와 Orca 번들
# (dist/orca)은 여기서 sync_assets.py 가 생성한다 — 이 스크립트는 정본만 쓴다.
CANON_PETS_DIR = os.path.join(REPO_ROOT, "assets", "pets")
...
EFFECTS_DIR = os.path.join(REPO_ROOT, "assets", "effects")
...
def compose_and_write(pet, rows, extra_manifest, row_order, frame):
    ...
    # 정본에만 쓴다. mac 미러(Resources/pets)와 Orca 번들(dist/orca)은
    # sync_assets.py 가 이 정본에서 만든다 (main() 끝에서 호출).
    canon_dir = os.path.join(CANON_PETS_DIR, pet["slug"])
    os.makedirs(canon_dir, exist_ok=True)
    sheet.save(os.path.join(canon_dir, "spritesheet.png"), optimize=True)
    with open(os.path.join(canon_dir, "pet.json"), "w") as fp:
        json.dump(manifest, fp, indent=2, ensure_ascii=False)

    print(f"wrote {canon_dir}/ ({sheet.size[0]}x{sheet.size[1]})")
...
def main():
    ...
    # 정본(assets/pets)을 갱신했으니 mac 미러와 Orca 번들을 다시 만든다.
    import subprocess
    subprocess.run([sys.executable, os.path.join(HERE, "sync_assets.py")], check=True)

...
- `.github/workflows/build-pet-dmg.yml` — `swift run`과 동일하게 **전체 펫**이 든 단일 앱을 `pet.app`/`pet.dmg`(실행 파일명도 `pet`)로 빌드하는 수동(`workflow_dispatch`, 입력 없음) CI 파이프라인. 펫을 고르거나 소스를 패치하지 않고 기본 빌드를 그대로 배포한다. Sparkle 임베드 + git 버전 주입 + (Secret 있으면) 서명된 `appcast.xml` 아티팩트 생성. 릴리스 다운로드 URL은 `releases/latest/download/pet.dmg`

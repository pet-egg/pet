...
- `apps/macos/` — 실제 결과물인 독립 macOS 앱 (Swift Package, Xcode 불필요)
  ...
  - `Sources/ConnorPet/ClaudeProcessActivity.swift` — (레거시) Claude 프로세스 트리 CPU 샘플러(`proc_pid_rusage`, mach 틱→ns timebase 변환). 예전 CPU 기반 데스크톱 감지에 쓰였으나 지금은 AX로 대체돼 **어디서도 참조 안 함** — 되돌리기용으로만 남겨 둠
  ...
  - `Sources/ConnorPet/SpriteSheet.swift`, `PetView.swift`(펫 아래 경험치 바 포함 + 우클릭 모션 메뉴 맨 아래 "방해금지 모드"·"설정…" 항목. 방해금지 토글은 `onToggleDND` 콜백으로 AppDelegate에 위임하고 체크 표시는 `dndEnabled`로 그린다), `PetWindow.swift` — 렌더링
  - `Sources/ConnorPet/SettingsWindow.swift` — 펫 우클릭 › "설정…"으로 여는 설정 창(`SettingsWindowController`). **메뉴바 아이콘이 노치/과밀로 가려 접근 못 하는 사용자를 위한 두 번째 진입점** — 메뉴바 아이템에 흩어져 있던 기능(펫 선택·상태 소스·진화/경험치 바 토글·경험치 초기화·Claude Code 훅·전체 디스크 접근·**방해금지 모드**·대전/노려보기·종료)을 한 창에 모았다. 대전/노려보기 섹션 맨 위에 방해금지 모드 스위치가 있고, 방해금지 중인 상대는 목록엔 그대로 두되 신청/노려보기 버튼을 잠그고 "방해금지 중 — 상호작용을 받지 않아요"로 이유를 밝힌다. 룩은 macOS 시스템 설정/Linear 계열 **무채색 그룹 카드**(둥근 카드 + 섹션 제목 + 행별 라벨/컨트롤), 색은 전부 시스템 semantic 그레이라 라이트/다크 자동 대응(켜짐 `NSSwitch`도 `CALayer.filters`+CoreImage 채도 제거로 accent 색 대신 항상 그레이). 동작은 `SettingsActionsDelegate`로 AppDelegate에 위임해 **메뉴바와 완전히 같은 코드 경로**를 탄다(어느 쪽에서 바꾸든 저장·메뉴바 갱신 동일). `CONNORPET_DEBUG_SETTINGS=<png경로> swift run`으로 레이아웃을 오프스크린 PNG로 덤프해 확인 가능
  - `Sources/ConnorPet/BattleService.swift` — 같은 wifi 발견(Bonjour, `NWListener`/`NWBrowser`) + 대전 신청/수락 핸드셰이크(length-prefixed JSON over `NWConnection`). **방해금지 모드**(`dndEnabled`)를 TXT 레코드(`txt["dnd"]`)로 광고하고 `BattlePeer.dnd`로 파싱해, 상대 앱이 신청 버튼을 미리 잠글 수 있게 한다(구버전은 dnd 필드가 없어 nil→꺼짐으로 봄). 켜져 있으면 들어오는 대전 신청·노려보기를 `handleInboundMessage`에서 조용히 끊는다(decline도 안 보냄 — "거절"이 아니라 "안 받음"이라 신청자 쪽은 무응답으로 마무리). **보내는 쪽(내가 거는 것)은 막지 않는다.** 라이브 갱신은 `updateDND(_:)`
...
- `apps/windows/` — 윈도우용 앱 (Python/PySide6). `pet_win/` 에 상태워처·XP·애니메이션·스프라이트 로딩을 포팅. 리소스는 `assets/pets` 를 직접 읽는다. `pet.spec`(PyInstaller)로 `pet.exe` 빌드. `pet_win/updater.py` = **인앱 자동 업데이터**(맥 Sparkle 대응, Windows frozen 전용): `releases/latest/download/windows-appcast.json`(버전·URL·서명)을 읽어 새 버전이면 `pet.exe` 를 내려받아 **맥과 같은 EdDSA(Ed25519) 공개키로 서명 검증**한 뒤(PyNaCl), 트레이 메뉴에서 사용자가 누르면 실행 중 exe 를 rename 으로 비켜 두고 교체·재실행한다. 버전은 `pet_win/_version.py`(CI가 태그로 주입). 자세한 건 `apps/windows/README.md`
...
- `.github/workflows/build-release.yml` — **하나의 `v*` 태그로 맥과 윈도우를 함께 빌드해 한 릴리스에 올리는 통합 파이프라인**. `build-macos`(Swift→`pet.app`/`pet.dmg`, Sparkle 임베드+서명된 `appcast.xml`) + `build-windows`(PySide6→PyInstaller `pet.exe`) 를 병렬로 빌드하고, `release` 잡이 둘을 받아 **같은 릴리스에 `pet.dmg`·`pet.exe` 를 함께 업로드**(`--latest`). 플랫폼 구분은 태그가 아니라 파일 이름(맥=`pet.dmg`, 윈도우=`pet.exe`). 다운로드 URL: `releases/latest/download/pet.dmg`(맥·Sparkle) / `releases/latest/download/pet.exe`(윈도우·install.ps1). `publish-appcast` 잡이 `appcast.xml` 을 GitHub Pages 로 배포. `release` 잡은 **같은 `SPARKLE_EDDSA_PRIVATE_KEY` 로 `pet.exe` 도 Ed25519 서명**해 `windows-appcast.json`(윈도우 인앱 업데이터가 읽음)을 만들어 릴리스에 올린다(키 없거나 형식 불일치면 릴리스는 막지 않고 매니페스트만 생략 → 그 릴리스만 윈도우 자동 업데이트 비활성). **트리거**: `v*` **태그 푸시**만. 릴리스에는 `SPARKLE_EDDSA_PRIVATE_KEY` 시크릿 필수
- `install.sh` / `install.ps1` — 최종 사용자용 원라이너 설치기. 맥은 `curl … | bash`(dmg→/Applications→quarantine 제거), 윈도우는 `irm … | iex`(pet.exe→`%LOCALAPPDATA%\pet`→`Unblock-File` 로 MOTW 제거→시작메뉴 바로가기). 둘 다 `PET_CHANNEL=stable` 로 안정화 채널 지원, 이미 있으면 종료 후 교체(업데이트 겸용)
...
- **모든 변경은 맥·윈도우 동시개발**: 이제 이 저장소는 맥(`apps/macos`, Swift)과 윈도우(`apps/windows`, Python/PySide6)를 **하나의 릴리스(`v*` 태그 → dmg+exe 동시 빌드)로 함께 배포**한다. 따라서 **사용자에게 보이는 동작·상태 로직·펫/에셋·XP·진화 등 공통 기능을 바꾸면 두 앱에 같이 반영**하고, 계약 문서 `shared/BEHAVIOR.md`(+`pet-manifest.schema.json`)도 함께 갱신할 것. 새 펫은 세 곳(mac `availablePetSlugs` · win `petmeta.AVAILABLE_PET_SLUGS`/`EVOLUTION_CHAINS` · `build_sheet.py` `PETS`), 상태/우선순위/XP 임계치는 `PetAnimationState.swift`↔`animation.py` / `TokenUsage.swift`↔`{tokenusage,xpmodel}.py` 처럼 **짝을 맞춰** 수정한다. **한쪽만 고치는 건 원칙적으로 금지** — 플랫폼 전용(맥의 Sparkle·AX·대전, 윈도우의 설치 스크립트 등)이라 한쪽에만 해당하는 변경은 PR 본문에 그 이유를 명시한다. mac 은 `swift run`, win 은 `QT_QPA_PLATFORM=offscreen python -m pytest tests` 로 각각 확인.
...
- **버전은 SemVer(`vX.Y.Z`) — 기능=MINOR, 버그수정/작은 변경=PATCH**: 릴리스는 `v*` 태그가 단일 소스이고(태그 푸시로만 CI가 빌드·배포한다), **하나의 태그가 맥(`pet.dmg`)과 윈도우(`pet.exe`)를 함께 빌드해 같은 릴리스에 올린다**(플랫폼별 태그를 따로 두지 않는다 — 위 "맥·윈도우 동시개발"과 짝). 다음 규칙으로 다음 태그를 정한다:
  ...
  - **"안정화(stable)" = 직전 마이너 라인의 최고 패치**: 최신은 방금 나온 마이너라 패치가 안 쌓여 검증이 덜 된 반면, **바로 이전 마이너 라인의 마지막 패치**는 버그 수정이 충분히 누적돼 검증됐다고 본다(예: 최신 `v1.6.0`, stable = `v1.5.4`). `install.sh`(맥)/`install.ps1`(윈도우) 는 `PET_CHANNEL=stable`(또는 `--stable`)이면 GitHub 릴리스 목록에서 이 버전을 계산해 그 태그의 `pet.dmg`(`releases/download/<tag>/pet.dmg`)를 받는다. 기본(무옵션)은 종전대로 최신(`releases/latest`)이다. 자세한 건 README "자동 업데이트 (Sparkle)" › "버전 규칙과 안정화 버전 받기" 참고.
- **커밋 메시지는 항상 한글로 작성**: 제목/본문 모두 한글로 쓸 것 (`Co-Authored-By:` 트레일러 등 고정 형식 줄은 예외).
- **PR 생성 시 assignees에 작성자를 넣을 것**: `gh pr create` 로 PR을 열 때 `--assignee @me`(또는 작성자 계정)를 붙여 담당자가 비어 있지 않게 한다. 누가 낸 PR인지 목록에서 바로 보이고, 리뷰·후속 조치의 책임 소재가 분명해진다.
- **모든 상호작용은 말풍선으로, 모달을 바로 띄우지 않을 것 (업무 방해 최소화)**: 노려보기·대전 신청 등 **남이 거는 상호작용**이나 알림은 갑작스러운 가운데 모달(`BattleDialog.info`/`.challenge` 등)로 바로 튀어나오지 않게 한다. 기본은 펫 **말풍선**(`petView?.enqueueCelebration(...)` / `SpeechBubbleWindow`)이나 펫 옆 작은 클릭-가능 말풍선(`ChallengeBubbleWindow`·`StareBubbleWindow`)·카운트다운으로 먼저 알리고, 모달이 꼭 필요하면(수락/거절·상세 보기처럼) **사용자가 그 말풍선을 눌렀을 때만** 뜨게 한다. 예: 대전 신청의 "Challenge" 말풍선→수락/거절 모달, 노려보기의 펫 도트 말풍선(`StareBubbleWindow`, 말풍선 안에 상대 펫 얼굴 크롭을 함께 표시)→얼굴 큼직한 모달. 이 앱은 사용자가 업무 중 배경에 띄워 두는 것이라, 포커스를 뺏는 모달이 바로 뜨면 그 자체로 방해가 된다.
